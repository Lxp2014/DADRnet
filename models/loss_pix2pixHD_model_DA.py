...
class Pix2PixHDModel(BaseModel):
    ...
    def init_loss_filter(self, use_gan_feat_loss, use_vgg_loss):
        flags = (True, use_gan_feat_loss, use_vgg_loss, True, True, True, True, True, True, True, True, True, True, True, True, True, True, True)

        def loss_filter(g_gan, g_gan_feat, g_vgg, g_kl, d_real, d_fake, g_featd, featd_real, featd_fake, diff, g_gan_dec, g_gan_feat_dec, g_vgg_dec, d_real_dec, d_fake_dec, smooth_l1_loss, g_dark_chanl, g_total_vari):
            return [l for (l, f) in zip((g_gan, g_gan_feat, g_vgg, g_kl, d_real, d_fake, g_featd, featd_real, featd_fake, diff,
                                         g_gan_dec, g_gan_feat_dec, g_vgg_dec, d_real_dec, d_fake_dec, smooth_l1_loss, g_dark_chanl, g_total_vari), flags) if f]
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.loss_names = self.loss_filter('G_GAN', 'G_GAN_Feat', 'G_VGG', 'G_KL', 'D_real', 'D_fake', 'G_featD', 'featD_real','featD_fake','Diff', 'G_GAN_dec','G_GAN_Feat_dec','G_VGG_dec','D_real_dec','D_fake_dec','smooth_l1_loss', 'g_dark_chanl', 'g_total_vari')
    ...
    def DCLoss(self, img, patch_size): #-------------------------------------------------------------------------------
        ...
        loss = torch.nn.L1Loss(size_average=True)(-dc, target)
        return loss
    ...
    def forward(self, label, inst, image, feat, real_ha, infer=False):
        ...
        fake_image_real = self.netG.forward(real_hiddens + hiddens_real + noise, 'dec')   #xiugai--------------------------------------------------------

        fake_image_sys = self.netG.forward(hiddens + hiddens_sys + noise, 'dec')            #xiugai--------------------------------------------------------

        label_feat_map = self.mapping_net(hiddens.detach())
        fake_image_clean=self.Privatesys_netG.forward(label_feat_map + noise, 'dec')
        label_feat_map_real = self.mapping_net(real_hiddens.detach())
        ...
        #         private_real_old_feat.append(hiddens_real[index].unsqueeze(0))
        #         fake_image_temp= fake_image_real[index].unsqueeze(0)
        #
        #     else:
        #         syn_feat.append(hiddens[index].unsqueeze(0))
        #         private_sys_old_feat.append(hiddens_sys[index].unsqueeze(0))
        #         fake_image_temp= fake_image_sys[index].unsqueeze(0)
        #     fake_image.append(fake_image_temp)
        # L=min(len(real_old_feat),len(syn_feat))
        # real_old_feat=real_old_feat[:L]
        # syn_feat=syn_feat[:L]
        # private_real_old_feat = private_real_old_feat[:L]
        # private_sys_old_feat = private_sys_old_feat[:L]
        #
        # real_old_feat=torch.cat(real_old_feat,0)
        # syn_feat=torch.cat(syn_feat,0)
        # private_real_old_feat = torch.cat(private_real_old_feat,0)
        # private_sys_old_feat = torch.cat(private_sys_old_feat,0)
        #
        ...
        if self.opt.no_cgan:
            ...
            # trying to discriminate the fake images and the real images
            pred_fake_pool_sys = self.discriminate(None, fake_image_sys, use_pool=True)
            pred_fake_pool_real = self.discriminate(None, fake_image_real, use_pool=True)
            #print('pred_fake_pool_sys:',pred_fake_pool_sys.shape)
            loss_D_fake = 0.5* (self.criterionGAN(pred_fake_pool_sys, False) + self.criterionGAN(pred_fake_pool_real, False))
            # Real Detection and Loss
            pred_real_sys = self.discriminate(None, input_label)
            pred_real_real = self.discriminate(None, real_haze)
            loss_D_real = 0.5* (self.criterionGAN(pred_real_sys, True) + self.criterionGAN(pred_real_real, True))
            # trying to fool the discriminator not to discriminate the fake and the real
            # GAN loss (Fake Passability Loss)
            pred_fake_sys = self.netD.forward(fake_image_sys)
            pred_fake_real = self.netD.forward(fake_image_real)
            #print('pred_fake_sys:',pred_fake_sys.shape)
            loss_G_GAN = 0.5* (self.criterionGAN(pred_fake_sys, True) + self.criterionGAN(pred_fake_real, True))
      ...
      #  loss_G_GAN_Feat = 0
        ...
        if not self.opt.no_ganFeat_loss:
            ...
            loss_G_GAN_Feat= 0.5 * (loss_G_GAN_Feat_sys+loss_G_GAN_Feat_real)
    ...
    #    loss_G_VGG_sys = 0
     #   loss_G_VGG_real = 0
        ...
        if not self.opt.no_vgg_loss:
            ...
            loss_G_VGG = loss_G_VGG_sys + loss_G_VGG_real
        # Only return the fake_B image if necessary to save BW
            fake_image=fake_image_sys

        # Dark channel loss and total variational loss by yuhu -----------------------------------------------
        loss_dark_channel_syn = self.DCLoss((fake_image_clean + 1) / 2, 35) * 0.01
        loss_dark_channel_real = self.DCLoss((fake_image_real_clean + 1) / 2, 35) * 0.01
        loss_dark_channel = 0.5*(loss_dark_channel_syn+loss_dark_channel_real)
        # also can be the self.TVLossL1
        loss_total_variation_syn = self.TVLoss(fake_image_clean) * 0.001
        loss_total_variation_real = self.TVLoss(fake_image_real_clean) * 0.001
        loss_total_variation = 0.5*(loss_total_variation_syn+loss_total_variation_real)
        #-----------------------------------------------------------------------------------------------------
        smooth_l1_loss=0
        if self.opt.Smooth_L1:
            smooth_l1_loss=self.criterionImage(fake_image_clean,real_image)*self.opt.L1_weight

        return [self.loss_filter(loss_G_GAN, loss_G_GAN_Feat, loss_G_VGG, loss_G_kl, loss_D_real, loss_D_fake,loss_G_featD, loss_featD_real, loss_featD_fake, loss_dif, loss_G_GAN_dec,loss_G_GAN_Feat_dec,loss_G_VGG_dec,loss_D_real_dec,loss_D_fake_dec,smooth_l1_loss,loss_dark_channel,loss_total_variation),
                None if not infer else fake_image, fake_image_clean, fake_image_real, fake_image_real_clean]
